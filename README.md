...
```bash
k6 run --vus 10 --duration 30s tests/api/register-user.js
```
...
|---|---|
| `register-user.js` | Registers a new user account via the `account/register` endpoint. |
| `login-user.js` | Registers a new user and then logs in via the `account/login` endpoint. |
| `logout-user.js` | Registers a user and logs out via the `account/logout` endpoint. |
| `fetch-customer.js` | Registers a user and fetches the customer profile via `account/customer`. |
| `change-profile.js` | Registers a user and updates their profile via `account/change-profile`. |
| `list-addresses.js` | Registers a user and lists their addresses via `account/list-address`. |
...
| `guest-order-store-api.js` | Full guest checkout flow: creates context, adds product to cart, registers as guest, and places an order. Supports `PRODUCT_QUANTITY` env variable. |
| `fetch-orders.js` | Places an order then lists customer orders via the `order` endpoint. |
| `cancel-order.js` | Places an order and cancels it via `order/state/cancel`. |
| `handle-order-payment.js` | Places an order and triggers payment handling via `order/payment`. |
| `fetch-products.js` | Fetches a product list via the `product` endpoint. |
| `fetch-product-detail.js` | Fetches a random product detail page via `product/{productId}`. |
| `fetch-product-listing.js` | Fetches a product listing for a random category via `product-listing/{categoryId}`. |
| `search-products.js` | Searches for products using a random keyword via the `search` endpoint. |
| `search-suggest.js` | Fetches search suggestions using a random keyword via `search-suggest`. |
| `fetch-category.js` | Fetches a random category listing page via the `category/{id}` endpoint. |
| `fetch-categories.js` | Fetches the category list/tree via the `category` endpoint (no ID). |
| `fetch-navigation-categories.js` | Fetches the main navigation and then fetches each child category via the `navigation` and `category` endpoints. |
| `fetch-cms-page.js` | Fetches a category to find its CMS page ID, then fetches the CMS page via `cms/{id}`. |
| `fetch-landing-page.js` | Fetches a landing page via `landing-page/{id}`. Requires `LANDING_PAGE_ID` env variable. |
| `fetch-breadcrumb.js` | Fetches the breadcrumb for a random category via `breadcrumb/{id}`. |
| `fetch-payment-methods.js` | Creates a context and fetches available payment methods via the `payment-method` endpoint. |
| `fetch-shipping-methods.js` | Creates a context and fetches available shipping methods via the `shipping-method` endpoint. |
| `fetch-wishlist.js` | Registers a user and fetches their wishlist via `customer/wishlist`. |
| `add-to-wishlist.js` | Registers a user and adds a product to the wishlist via `customer/wishlist/add/{productId}`. |
| `delete-from-wishlist.js` | Registers a user, adds a product to wishlist, then removes it via `customer/wishlist/delete/{productId}`. |
| `fetch-countries.js` | Fetches the list of countries via the `country` endpoint. |
| `fetch-country-states.js` | Fetches states for a country via `country-state/{countryId}`. |
| `fetch-currencies.js` | Fetches the list of currencies via the `currency` endpoint. |
| `fetch-languages.js` | Fetches the list of languages via the `language` endpoint. |
| `fetch-salutations.js` | Fetches the list of salutations via the `salutation` endpoint. |
| `fetch-cookie-groups.js` | Fetches cookie consent groups via the `cookie-groups` endpoint. |
