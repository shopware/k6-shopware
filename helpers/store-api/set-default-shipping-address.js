...
export function setDefaultShippingAddressViaStoreApi(trend, counter) {
  const flowStart = Date.now();
  const email = `k6-defship-${Date.now()}-${randomString()}@example.com`;
  const baseHeaders = {
  ...
  const resp = http.patch(
    `${salesChannel[0].url}/store-api/account/address/default-shipping/${addressId}`,
    null,
    {
      headers: withContextHeaders(contextToken),
      tags: { name: "api.account.address.default_shipping" },
    }
  );
  ...
  const success = check(resp, {
    "Store API set default shipping address is successful": (r) =>
      r.status === 200 || r.status === 204,
  });

  if (!success) {
    console.log(`Store API set default shipping address failed: ${resp.body}`);
  }
