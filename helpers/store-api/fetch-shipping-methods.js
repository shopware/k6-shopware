...
export function fetchShippingMethodsViaStoreApi(trend, counter) {
  const baseHeaders = {
  ...
  const resp = http.post(
    ...
    {
      headers,
      tags: { name: "api.checkout.shipping_method.fetch" },
    }
  );
  ...
  const success = check(resp, {
    "Store API fetch shipping methods is successful": (r) =>
      r.status === 200,
  });

  if (!success) {
    console.log(`Store API fetch shipping methods failed: ${resp.body}`);
  }
