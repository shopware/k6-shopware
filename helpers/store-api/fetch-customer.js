...
export function fetchCustomerViaStoreApi(trend, counter) {
  const flowStart = Date.now();
  const email = `k6-customer-${Date.now()}-${randomString()}@example.com`;
  const baseHeaders = {
  ...
  const resp = http.post(
    `${salesChannel[0].url}/store-api/account/customer`,
    "{}",
    {
      headers: withContextHeaders(contextToken),
      tags: { name: "api.account.customer.fetch" },
    }
  );
  ...
  const success = check(resp, {
    "Store API fetch customer is successful": (r) => r.status === 200,
  });

  if (!success) {
    console.log(`Store API fetch customer failed: ${resp.body}`);
  }
