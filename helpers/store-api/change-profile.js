...
import { getChangeProfilePayload } from "./payloads/change-profile.js";

export function changeProfileViaStoreApi(trend, counter) {
  const flowStart = Date.now();
  const email = `k6-profile-${Date.now()}-${randomString()}@example.com`;
  const baseHeaders = {
  ...
  const payload = getChangeProfilePayload({
    salutationId: salesChannel[0].salutationIds[0],
  });

  const resp = http.post(
    `${salesChannel[0].url}/store-api/account/change-profile`,
    JSON.stringify(payload),
    {
      headers: withContextHeaders(contextToken),
      tags: { name: "api.account.change_profile" },
    }
  );
  ...
  const success = check(resp, {
    "Store API change profile is successful": (r) => r.status === 200,
  });

  if (!success) {
    console.log(`Store API change profile failed: ${resp.body}`);
  }
