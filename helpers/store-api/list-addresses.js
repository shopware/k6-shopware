...
export function listAddressesViaStoreApi(trend, counter) {
  const flowStart = Date.now();
  const email = `k6-listaddr-${Date.now()}-${randomString()}@example.com`;
  const baseHeaders = {
  ...
  const resp = http.post(
    `${salesChannel[0].url}/store-api/account/list-address`,
    "{}",
    {
      headers: withContextHeaders(contextToken),
      tags: { name: "api.account.list_address" },
    }
  );
  ...
  const success = check(resp, {
    "Store API list addresses is successful": (r) => r.status === 200,
  });

  if (!success) {
    console.log(`Store API list addresses failed: ${resp.body}`);
  }
