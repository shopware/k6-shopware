...
export function fetchPaymentMethodsViaStoreApi(trend, counter) {
  const baseHeaders = {
  ...
  const resp = http.post(
    ...
    "{}",
    {
      headers,
      tags: {
        name: "api.checkout.payment_method.fetch",
      },
    }
  );
  ...
  const success = check(resp, {
    "Store API fetch payment methods is successful": (r) => r.status === 200,
  });

  if (!success) {
    console.log(`Store API fetch payment methods failed: ${resp.body}`);
  }
