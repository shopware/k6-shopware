...
import { getCancelOrderPayload } from "./payloads/cancel-order.js";

export function cancelOrderViaStoreApi(trend, counter) {
  const flowStart = Date.now();
  const email = `k6-cancel-${Date.now()}-${randomString()}@example.com`;
  const baseHeaders = {
  ...
  if (!orderId) {
    console.log(
      `Store API cancel order skipped: no orderId from order response (status=${orderResp.status})`
    );
  ...
  const resp = http.post(
    `${salesChannel[0].url}/store-api/order/state/cancel`,
    JSON.stringify(getCancelOrderPayload({ orderId })),
    {
      headers: withContextHeaders(contextToken),
      tags: { name: "api.order.state.cancel" },
    }
  );
  trend.add(Date.now() - flowStart);
  counter.add(1);
  ...
  const success = check(resp, {
    "Store API cancel order is successful": (r) => r.status === 200,
  });

  if (!success) {
    console.log(`Store API cancel order failed: ${resp.body}`);
  }
